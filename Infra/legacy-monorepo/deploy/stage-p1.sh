#!/usr/bin/env bash
# SUPERSEDED — staging host decommissioned (ADR-0025); references to
...
#
# Usage:
#   sudo ./stage-p1.sh /home/deploy/taha-stage/release-<version>-<hash>
#
# Safety:
...
marker = "staging.tahamohamadi.ir {"
idx = text.find(marker)
if idx == -1:
    print("error: staging block not found in Caddyfile", file=sys.stderr)
    sys.exit(1)

prefix = text[:idx].rstrip() + "\n"
new_block = """staging.tahamohamadi.ir {
