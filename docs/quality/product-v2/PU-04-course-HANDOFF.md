# PU-04-course Delivery Handoff

Owner repository: `Back-End` (`d:\Project\tahamohammadi-platform\Back-End`)  
Packet: `PU-04-course`  
Specification: `Docs/05-delivery/concept-alignment-v2/product-packets/PU-04-course.md`  
...
Attached nullable localized story composition to `Course`, added validation and schema exposure for admin `storyId`, and exposed sanitized story in the public detail endpoint per §I03:

- **Model Relation (`apps/content/models.py`)**:
  - Added nullable `story` ForeignKey to `Course`:
    ```python
    ...
    ```
- **Database Migration (`apps/content/migrations/0023_pu_course_story.py`)**:
  - Created migration adding `story_id` column to `content_course`.
  - Validated forward migration (`0023`), rollback (`0022`), and re-application (`0023`) on database without data loss.
- **Admin Schema & Validation (`apps/api/admin_content.py`)**:
  - Added `"story": "storyId"` to `DETAIL_FIELD_MAPS["course"]`, exposing `storyId` in generic admin schema `/api/v1/admin/content/schema` with type `"number"`.
...
- **Public API Projection (`apps/api/api.py`)**:
  - Added `story: StoryDocumentOut | None = None` to `CourseDetailOut`.
  - Implemented `resolve_story` delegating to `public_story_document(getattr(obj, "story", None), obj.locale)`.
  - Draft isolation: returns `None` if the story is unpublished, draft, non-story, or if the story locale mismatches the course locale.
  - Sanitization: filters disabled sections/blocks, sanitizes rich text and MathML, resolves active public media.
- **Test Suite (`tests/test_product_course_story.py`)**:
  - Implemented 6 unit and integration tests covering:
    ...
    - Public course detail exposure of sanitized story document.
...
- `apps/content/models.py` (MODIFIED): Added `story` foreign key to `Course`.
- `apps/content/migrations/0023_pu_course_story.py` (NEW): Database migration adding `story_id` column.
- `apps/api/api.py` (MODIFIED): Added `story` field and resolver to `CourseDetailOut`.
- `apps/api/admin_content.py` (MODIFIED): Added `"story": "storyId"` mapping and locale validation.
- `tests/test_product_course_story.py` (NEW): Test suite covering model, admin validation, and public projection.
- `docs/contracts/openapi/current/public-openapi.json` (MODIFIED): Re-exported schema reflecting `CourseDetailOut.story`.
- `docs/contracts/openapi/current/admin-openapi.json` (MODIFIED): Re-exported schema reflecting `course` schema `storyId`.
...
- `docs/quality/product-v2/PU-04-course-HANDOFF.md` (NEW): This delivery report.
...
```bash
uv run pytest tests/test_product_course_story.py
```
**Output**: `6 failed in 2.98s`
- `TypeError: Course() got unexpected keyword arguments: 'story'`
- `AssertionError: Course must define a 'story' relation`
- `AssertionError: assert 'storyId' in course_fields`

### 3.2 Targeted Test Suite
```bash
uv run pytest tests/test_product_course_story.py -vv
```
**Result**: `6 passed in 2.82s`
- `test_course_model_has_story_field`: PASSED
- `test_admin_content_schema_exposes_course_story_id`: PASSED
- `test_admin_crud_course_story_id`: PASSED
- `test_admin_course_story_validation`: PASSED
- `test_public_course_detail_exposes_story`: PASSED
- `test_public_course_detail_draft_isolation`: PASSED

### 3.3 Regressions (Publication & Metadata)
```bash
uv run pytest tests/test_product_publication_story.py tests/test_product_metadata.py -vv
```
**Result**: `13 passed in 2.88s`
...
```
**Result**:
- `Unapplying content.0023_pu_course_story... OK`
- `Applying content.0023_pu_course_story... OK`
...
- `public-openapi.json`:
  - Paths: 42
  - SHA256: `677708afbf39428642de6453a99b05b128563fcfce0a83c978e9f91d3ac81094`
- `admin-openapi.json`:
  - Paths: 51
...
- **Draft Isolation**: Attached story composition is only included in public detail if `story.status == "published"` and `story.locale == course.locale`. Otherwise, `story` resolves to `None`.
...
2. **Shared File Coupling**:
   - `apps/api/api.py`, `apps/api/admin_content.py`, and `apps/content/models.py` contain dirty uncommitted changes across predecessor packets (`PU-03-resolver`, `PU-03-settings`, `PU-04-catalog`, `PU-04-metadata`, `PU-04-publication`, `PU-04-course`).
